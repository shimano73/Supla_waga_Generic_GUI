...
SuplaConfigESP::SuplaConfigESP() {
  ...
  if (ConfigManager->isDeviceConfigured()) {
    ...
    if (String(ConfigManager->get(KEY_LPG_CALIBRATIONS)->getValue()) == 0) {
      ConfigManager->set(KEY_LPG_CALIBRATIONS, DEFAULT_LPG_CALIBRATIONS);
    }
    if (String(ConfigManager->get(KEY_LPG_EMPTY)->getValue()) == 0) {
      ConfigManager->set(KEY_LPG_EMPTY, DEFAULT_LPG_EMPTY);
    }
    if (String(ConfigManager->get(KEY_LPG_FULL)->getValue()) == 0) {
      ConfigManager->set(KEY_LPG_FULL, DEFAULT_LPG_FULL);
    }
...
void SuplaConfigESP::factoryReset() {
  ...
  if (!digitalRead(0)) {
    ...
    ConfigManager->set(KEY_LPG_CALIBRATIONS, "10");
    ConfigManager->set(KEY_LPG_EMPTY, "10");
    ConfigManager->set(KEY_LPG_FULL, "10");
